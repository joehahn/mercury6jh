c
c mercury6.2.jh is John Chamber's mercury6_2.for code, but tweaked slightly
c to suit my purposes. Those tweaks are:
c
c #1. Don't generate ce.out, which is the file that stores
c close-encounter data. Crowded simulations tended to produce a large
c ce.out file, so this is disabled in MCE_INIT(), MIO_IN(), and MIO_CE().
c
c #2. The screen output generated by MIO_LOG() is modified so that you 
c can estimate total runtimes based on your simulation's progress.
c The planets' radii are also printed in the info.out file.
c
...
c
c MFO_USER() can also output data to file 'user.out', as needed,
c usually for testing purposes.
c
c #4. Added DRAG() subroutine, which, if called, causes a body's 
c semimajor axis, eccentricity, and inclination to decay at rates 
c proportional to p1,p2,p3. This subroutine also calls 
c RX(), RY(), RZ(), which are used to rotate a coordinate system.
c
c Also added MIGRATE() subroutine, which causes planets
c orbits to migrate a radial distance da=p2 over an
c efold-timescale tau=p1.
c
c #5. Input four floating point global parameters via the param.in 
c file; see lines containing global parameter1 = value, etc).
c These values are passed to MFO_USER() via array global_params(4),
c and they are used to control user-defined forces, like, say, an
c acceleration due to a Galactic tide.
c
c Send any comments, suggestions, and bug-reports on these particular
c tweaks to me at the  address below.
c
c Joe Hahn
c Space Science Institute
c jhahn@spacescience.org
c April 2007
c
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
c  OPT(7) = apply post-Newtonian correction? is disabled
...
c 
c MFO_USER() can also write data to file 'user.out' at each output
c interval, usually for testing purposes.
c
c Tweak #5 allows the array global_params(4) to communicate 
c global parameter1 through global parameter4 (whose values are
c given in param.in). These parameters can then be used to
c account for other user-defined forces, like, say, the Galactic tide.
c
...
cc     output data to file 'user.out', as needed
c      if (output_flag .gt. 0) then
c        open (unit=1, file='user.out', access='append')
c        write(1,*) time/365.25d0, global_params(1), global_params(2),
c     %    global_params(3), global_params(4)
c        close(unit=1)
c      end if

cc     drag accelerations
c      call drag(nbod,m,x,v,a,params)

cc     migration accelerations
c      call migrate(time,nbod,m,x,v,a,params)
c
c------------------------------------------------------------------------------
c
      return
...
c Author: Joe Hahn
c
c Calculate the cartesian components of the acceleration, a_xyz,
c that each particle experiences due to a generic drag acceleration.
c This drag acceleration is constructed so that (i.) the body's
c semimajor axis varies at the time-averaged rate da/dt=-alpha*a*n, 
c (ii.) its time-averaged eccentricity varies as 
c de/dt=-(beta-0.75*alpha)*e*n, and (iii.) its inclination varies as 
c di/dt=-gamma*i*n, where n=body's mean motion, and alpha=p1, 
c beta=p2, gamma=p3 are its optional input parameters that default to zero.
c
c------------------------------------------------------------------------------
c
      subroutine drag (nbod,m,xyz,v_xyz,a_xyz,params)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      integer nbod
      real*8 m(nbod),xyz(3,nbod),v_xyz(3,nbod),a_xyz(3,nbod)
      real*8 params(3,nbod)
c
c Local
      integer j
      real*8 x,y,z,vx,vy,vz,gm,q,e,i,wt,O,MA,sma,w,Ec
      real*8 tan_fover2,f,n,rdot,alpha,beta,gamma
      real*8 a_r,a_t,ax,ay,az,mco_kep
c
c------------------------------------------------------------------------------
c
c     acceleration on all other bodies
      do j = 2, nbod

c       particle's coordinates
        x=xyz(1,j)
        y=xyz(2,j)
        z=xyz(3,j)
        vx=v_xyz(1,j)
...
c       get particle's orbit elements, where sma=semimajor axis,
c       e=ecc, i=inc, w=arg or peri, O=longitude of node,
c       MA=mean anomaly
        gm = m(1)+m(j)
        call mco_x2el(gm,x,y,z,vx,vy,vz,q,e,i,wt,O,MA)
        sma=q/(1.d0-e)
        w=wt-O

c       proceed if the orbit is an ellipse
        if (sma .gt. 0.d0) then
...
c         get true anomaly
          tan_fover2=sqrt((1.d0+e)/(1.d0-e))*tan(Ec/2.d0)
          f=2.d0*atan(tan_fover2)

c         mean motion
          n=sqrt(gm/(sma*sma*sma))
c          write(6,*) j, sma, n
c          write(6,*) j, sma*n, sqrt(vx**2+vy**2+vz**2)

c         radial velocity
          rdot=e*sma*n*sin(f)/sqrt(1.d0-e*e)

c         extract drag parameters
          alpha = params(1, j)
          beta  = params(2, j)
          gamma = params(3, j)

c         radial & tangential accelerations in the orbit plane
          a_r=-(2.d0)*beta*rdot*n
          a_t=-alpha*sma*n*n/2.d0

c         cartesian components of acceleration, in orbit-plane 
c         coordinates, which has x-y plane in the orbit plane,
c         with x pointing towards periapse
          ax=a_r*cos(f)-a_t*sin(f)
          ay=a_r*sin(f)+a_t*cos(f)
          az=0.d0

c         rotate coordinate system to reference-plane coordinates
          call Rz(-w, ax, ay, az, ax, ay, az)
          call Rx(-i, ax, ay, az, ax, ay, az)
          call Rz(-O, ax, ay, az, ax, ay, az)

c         add vertical acceleration to drive inclination-damping
          az=az-(2.d0)*gamma*vz*n

c         increment accelerations due to drag force
          a_xyz(1,j)=a_xyz(1,j)+ax
          a_xyz(2,j)=a_xyz(2,j)+ay
          a_xyz(3,j)=a_xyz(3,j)+az
c          write(6,*) j, a_xyz(1,j),a_xyz(2,j),a_xyz(3,j)
        end if
...
c Author: Joe Hahn
c
c Rotate the coordinate system about the x-axis by angle theta (in radians).
c The input vector has cartesian components (x,y,z) in the un-rotated
c coordinate system, while output (xr,yr,zr) are the components of that 
c vector in the rotated coordinate system.
c
c------------------------------------------------------------------------------
c
      subroutine Rx (theta, x, y, z, xr, yr, zr)
c
      implicit none
      include 'mercury.inc'
c
c Input/Output
      real*8 theta, x, y, z, xr, yr, zr
c
c------------------------------------------------------------------------------
c
      xr =  x
...
c Author: Joe Hahn
c
c Rotate the coordinate system about the y-axis by angle theta (in radians).
c The input vector has cartesian components (x,y,z) in the un-rotated
c coordinate system, while output (xr,yr,zr) are the components of that 
c vector in the rotated coordinate system.
c
c------------------------------------------------------------------------------
c
      subroutine Ry (theta, x, y, z, xr, yr, zr)
c
      implicit none
      include 'mercury.inc'
c
...
c Author: Joe Hahn
c
c Rotate the coordinate system about the z-axis by angle theta (in radians).
c The input vector has cartesian components (x,y,z) in the un-rotated
c coordinate system, while output (xr,yr,zr) are the components of that 
c vector in the rotated coordinate system.
c
c------------------------------------------------------------------------------
c
      subroutine Rz (theta, x, y, z, xr, yr, zr)
c
      implicit none
      include 'mercury.inc'
c
...
c Tweak #2: Write big bodies radii (in units of km and distance r)
c in the info.out file
  20    open (23,file=outfile(3),status='old',access='append',err=20)
        write (23,'(/,a)') mem(77)(1:lmem(77))
	do j = 2, nbig
           r = sqrt(xh(1,j)**2+xh(2,j)**2+xh(3,j)**2)
	   write (23,'(I9, f15.1, e17.3)') j-1,rphys(j)*AU/1.0e5,
     %        rphys(j)/r
        end do
        write (23,'(/)')
        close (23)
...
c       TWEAK #3---set output_flag when MFO_USER() is to write output
        output_flag=1
c
c Beware: the integration may change direction at this point!!!!
...
c
c Tweak #1: DON'T open or write to the close-encounter data file ce.out
...
c
c Tweak #3 is also implemented here, by writing each body's p1,p2,p3 parameters
c to the dump file
c------------------------------------------------------------------------------
c
...
c
c Also implement Tweak #1: Don't write/open j=2 close-encounter file ce.out,
c and Tweak #3: read body's optional parameters p1,p2,p3
c
...
c
c       Line below is the original code
c        do j = 1, 2
c Tweak #1: Don't write/open j=2 close-encounter file ce.out
        do j = 1, 1 
...
c
c Tweak #2: add clock information to the screen output
c
c------------------------------------------------------------------------------
c
...
c
      if (opt(3).eq.0.or.opt(3).eq.2) then
        tstring = mem(1)
        flog = 
     %'(i4,a,i2,a,i2,a,i2,a,i2,a,i2,4x,a,f14.1,a,2(a,1p1e12.5))'
      else if (opt(3).eq.1) then
        flog =
     %'(i4,a,i2,a,i2,a,i2,a,i2,a,i2,4x,a,i10,1x,i2,1x,f4.1,
     %2(a,1p1e12.5))'
      else
        tstring = mem(2)
        flog = 
     %'(i4,a,i2,a,i2,a,i2,a,i2,a,i2,4x,a,f14.3,a,2(a,1p1e12.5))'
      end if
c
      tmp0 = 0.d0
      tmp1 = 0.d0
...
c
     ...
     %    mem(64)(1:lmem(64)),year, month, t1,
